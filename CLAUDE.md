...
Everything else: the only local check is a YAML parse (`python3 -c "import yaml; yaml.safe_load(...)"`) plus `scripts/check-workflows.py`. The pre-push hook runs that and the guard suite as two independent checks, each probing for its own dependencies (PyYAML for the first, `jq` for the second) and skipping only itself — a single missing dependency must not take the other check down with it. Reusable workflows cannot be invoked from a local checkout — `uses:` resolves through GitHub. Real e2e test: push to feature branch, point one caller at it (`@feature/<name>`) before tagging.
